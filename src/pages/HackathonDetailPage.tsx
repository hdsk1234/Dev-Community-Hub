...
export const HackathonDetailPage = () => {
  ...
  // 🔥 [추가] 그린테크 챌린지일 경우에만 hackathon 변수를 비워서 아래 에러 UI가 뜨게 함
  if (slug === 'green-tech-challenge') {
    hackathon = undefined;
  }

  // 🔥 [수정] 요청하신 에러 UI 적용 (그린테크 챌린지 클릭 시 이 화면이 뜸)
  if (!hackathon) return (
    <div className="min-h-[60vh] flex flex-col items-center justify-center text-center">
      ...
      <Link to="/hackathons" className="px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold">
        목록으로 돌아가기
      </Link>
    </div>
  );
  ...
  // ---------------------------------------------------------
  // 아래는 원본 코드와 100% 동일 (나머지 해커톤 정상 작동용)
  // ---------------------------------------------------------

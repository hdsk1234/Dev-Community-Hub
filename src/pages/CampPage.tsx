...
export const CampPage = () => {
  ...
  const handleJoin = async (postId: string) => {
    ...
    try {
      await updateDoc(doc(db, 'camp_posts', postId), {
        members: arrayUnion(user.uid)
      });
      toast.success('팀에 합류했습니다!');
    } catch (error) {
      console.error('Error joining team:', error);
      toast.error('팀 합류에 실패했습니다.');
    }
  };
        ...
        <div className="lg:col-span-2 space-y-4">
          {posts.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
              <p className="text-slate-400">아직 등록된 모집글이 없습니다.</p>
            </div>
          ) : (
            posts.map((post, i) => (
              <motion.div
              ...
              >
                  ...
                  <div className="space-y-1">
                    ...
                    {post.hackathonSlug && (
                      <p className="text-xs text-indigo-500 font-bold mt-1">
                        #{MOCK_HACKATHONS.find(h => h.slug === post.hackathonSlug)?.title || post.hackathonSlug}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center space-x-2">
                    {(() => {
                      ...
                      return (
                        <a 
                          ...
                          className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl transition-all font-bold text-xs ${linkInfo.color}`}
                        >
                          {linkInfo.icon}
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-slate-50">
                  ...
                  <div className="flex items-center gap-2">
                    {user ? (
                      <button 
                        onClick={() => setActiveChat({ id: post.id, name: post.teamName })}
                        className="flex items-center space-x-1.5 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-bold hover:bg-indigo-100 transition-all"
                      >
                        <MessageSquare className="w-4 h-4" />
                        <span>팀 채팅</span>
                      </button>
                    ) : (
                      <p className="text-[10px] text-slate-400 font-medium italic">로그인 후 채팅 가능</p>
                    )}
                  </div>
                </div>
              </motion.div>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  ...
                  placeholder="팀의 목표와 분위기를 설명해주세요" 
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" 
                  placeholder="예: 프론트엔드, 디자이너, 기획자" 
                />
              </div>

              <div className="space-y-2">
                <label className="text-sm font-bold text-slate-700">연락 링크 (카톡 오픈채팅 또는 구글폼)</label>
                <input 
                  type="text" 
                  required
                  value={contactLink}
                  ...
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" 
                  placeholder="open.kakao.com... 또는 forms.gle..." 
                />
                <p className="text-[10px] text-slate-400">
                  * 카카오톡 오픈채팅(open.kakao.com) 또는 구글폼 링크만 등록 가능합니다.
                </p>
              </div>
